    ...
    try {
        ...
        try {
            ...
            if (dbSession) {
                ...
                console.log('✅ Recorded join in database for:', user_type);
            }
        } catch (dbError) {
        ...
        if (user_type === 'teacher') {
            ...
            agoraUid = 1;
            console.log('👑 Teacher assigned UID 1');
        } else {
            // Students get unique UIDs based on hash of their ID
            const hash = user_id.split('').reduce((a, b) => {
                a = ((a << 5) - a) + b.charCodeAt(0);
                return a & a;
            }, 0);
            agoraUid = Math.abs(hash % 100000) + 1000; // Range: 1000-101000
        ...
        const token = RtcTokenBuilder.buildTokenWithUid(
            ...
            agoraUid,
            RtcRole.PUBLISHER, // Both teachers and students can publish
            privilegeExpiredTs
        );
...
// Get session status (Works for both Teachers & Students)
router.get('/session-status/:meetingId', async (req, res) => {
    try {
        ...
        // Get session from memory first (for real-time status)
        const session = sessionManager.getSession(meetingId);
        
        if (session) {
            // Session found in memory - return real-time status
            ...
            console.log('✅ Session status from memory:', {
                meetingId,
        ...
        *,
        ...
        if (error || !dbSession) {
            console.log('❌ Session not found in database either');
            return res.status(404).json({ 
                success: false,
        ...
        // Calculate status from database session
        const participants = dbSession.video_session_participants || [];
        const teacherJoined = participants.some(p => p.is_teacher) || false;
        const studentCount = participants.filter(p => !p.is_teacher).length || 0;
        const isActive = dbSession.status === 'active' && !dbSession.ended_at;
        
        console.log('✅ Session status from database:', {
            meetingId,
            is_active: isActive,
        ...
        res.json({
            ...
            class_title: dbSession.class_title,
        ...
        // Return optimistic response to allow joining attempts
        res.json({
            success: false,
            error: 'Status check failed',
            is_active: true, // Optimistic - allow joining
            is_teacher_joined: false,
            student_count: 0,
            source: 'fallback'
        });
